...
include("hardsphere_correction_tests.jl")
...
function initABPE_ellipse(Np::Int64, L::Float64, R::Float64, v::Float64; T::Float64=300.0, η::Float64=1e-3)
    ...
    hardsphere_correction_cell!(xyθ[:,1:2],R) #xyθ[:,1:2] gives x and y positions of intitial particles
...
function multiparticleE(;Np::Integer, L::Float64, R::Float64, v::Float64, Nt::Int64=2, δt::Float64=1e-3)
    ...
    ABPE[1] = initABPE_square( Np, L, R, v ) # including initial hardsphere correction
...
function simulate!(ABPE, Nt, δt)
    # PΘ = [ (position(abpe), orientation(abpe)) ]
    # pθ = PΘ[1]
    
    for nt in 1:Nt
        ABPE[nt+1] = update(ABPE[nt], δt) #updating information at every step
        # if(nt % 50 == 0) println("timestep : $nt") end
        # println("Step $nt")
    end
    return nothing
end
...
function update(abpe::ABPE, δt::Float64) where {ABPE <: ABPsEnsemble}
    # 1) step
    ...
    # 3) collisions correction
    hardsphere_correction_cell!(pθ[1], abpe.R)
...
function multiparticleE_wall(;Np::Integer, L::Float64, R::Float64, v::Float64, Nt::Int64=2, δt::Float64=1e-3, wall_condition::String)
    ...
    if(wall_condition=="elliptical")
        ABPE[1] = initABPE_ellipse( Np, L, R, v ) # including initial hardsphere correction
    elseif(wall_condition in ["periodic", "squared"])
        ABPE[1] = initABPE_square( Np, L, R, v ) 
    else
        throw(ArgumentError("please provide a correct argument for wall condition"))
    end

    simulate_wall!(ABPE, Nt, δt, wall_condition)
    # println("I am in multiwall update")
    return position.(ABPE), orientation.(ABPE)
end

function simulate_wall!(ABPE, Nt, δt, wall_condition)
    # PΘ = [ (position(abpe), orientation(abpe)) ]
    # pθ = PΘ[1]
    
    for nt in 1:Nt
        if(nt%100==0) println("step $nt / $Nt") end
        ABPE[nt+1] = update_wall(ABPE[nt],δt, wall_condition)
        #println("Step $nt")
...
function update_wall(abpe::ABPE, δt::Float64, wall_condition::String) where {ABPE <: ABPsEnsemble}
