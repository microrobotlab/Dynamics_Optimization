...
function initABPE_ellipse(Np::Int64, L::Float64, R::Float64, v::Float64; T::Float64=300.0, η::Float64=1e-3)
    ...
    xyθ[:,1:2], dists, superpose, uptriang = hardsphere(xyθ[:,1:2],R) # xyθ[:,1:2] gives x and y positions of initial particles
...
function simulate!(ABPE, matrices, Nt, δt)
    for nt in 1:Nt
        ...
        if(nt % 100 == 0) println("timestep : $nt") end
    end
    return nothing
end
...
function hardsphere_correction!(xy::Array{Float64,2}, dists::Array{Float64,2}, superpose::BitArray{2}, R::Float64; tol::Float64=1e-3)
    ...
    for np1 in 1:Np
        ...
        if any(superpose[np1,:]) 
            ...
            # update distances and superpositions matrices
...
function hardsphere!(xy::Array{Float64,2}, dists::Array{Float64,2}, superpose::BitArray{2}, uptriang::BitArray{2}, R::Float64; tol::Float64=1e-3)
    ...
    while superpositions > 0
        ...
        # run `hardsphere_correction!` if superpositions remain
        if superpositions > 0
            hardsphere_correction!(xy,dists,superpose,R,tol=tol)
        end
        counter += 1
        # to avoid too much iterations 
        if counter >= 100
...
function periodic_BC_array!(xy::Array{Float64,2},L::Float64, R)   # when a particle crosses an edge it reappears on the opposite side
	...
	idx = abs.(xy[:,1]) .> L/2 + R          # Create vector idx in which we have 1 where the absolute value of the x coordinate of the particles is outside the observation area
	if any(idx)
		xy[idx,1] .-= sign.(xy[idx,1]).*L   # where we have uni in idx we make the particle reappear on the opposite side of x with respect to 0
	end
