...
#------------------------------------------------------------For ellipse ---------------------------------------------------------------------------------------------------------
function initABPE_ellipse(Np::Int64, R::Float64, v::Float64; T::Float64=300.0, η::Float64=1e-3)
...
#------------------------------------------------------------For square ---------------------------------------------------------------------------------------------------------
function initABPE_square(Np::Int64, R::Float64, v::Float64; T::Float64=300.0, η::Float64=1e-3, N::Integer, M::Integer, x_min, x_max, y_min, y_max)
    # translational diffusion coefficient [m^2/s] & rotational diffusion coefficient [rad^2/s] - R [m]
    # Intial condition will be choosen as per the geometry under study
    DT, DR = diffusion_coeff(1e-6R)
    ...
    #xyθ = (rand(Np,3).-0.0).*repeat([L L 2π],Np)
    xyθ[:,1:2], dists, superpose, uptriang = hardsphere(xyθ[:,1:2], R; N, M, x_min, x_max, y_min, y_max) #xyθ[:,1:2] gives x and y positions of intitial particles
...
function multiparticleE(;Np::Integer, L::Float64, R::Float64, v::Float64, Nt::Int64=2, δt::Float64=1e-3, N::Integer=16, M::Integer=16, x_min, x_max, y_min, y_max)
    ...
    ABPE[1], matrices = initABPE_square( Np, R, v; N, M, x_min, x_max, y_min, y_max) # including initial hardsphere correction
    
    simulate!(ABPE, matrices, Nt, δt; N, M, x_min, x_max, y_min, y_max)

    return position.(ABPE), orientation.(ABPE)
end

function simulate!(ABPE, matrices, Nt, δt; N, M, x_min, x_max, y_min, y_max)
    # PΘ = [ (position(abpe), orientation(abpe)) ]
    # pθ = PΘ[1]
    
    for nt in ProgressBar(1:Nt)
        ABPE[nt+1] = update(ABPE[nt],matrices,δt; N, M, x_min, x_max, y_min, y_max) #updating information at every step
        # println("Step $nt")
    end
    return nothing
end
...
function update(abpe::ABPE, matrices::Tuple{Matrix{Float64}, BitMatrix, BitMatrix}, δt::Float64; N::Integer, M::Integer, x_min, x_max, y_min, y_max) where {ABPE <: ABPsEnsemble}
    ...
    hardsphere!(pθ[1], matrices[1], matrices[2], matrices[3], abpe.R; N=N, M=M, x_min=x_min, x_max=x_max, y_min=y_min, y_max=y_max)
...
function hardsphere(
    ...
    hardsphere!(xy, dists, superpose, uptriang, R; tol=tol, N=N, M=M, x_min=x_min, x_max=x_max, y_min=y_min, y_max=y_max)
